...
// The waveOut playback design here, in particular the circular buffer,
// the silence played ahead of a new stream, and the way finished
// buffers are returned from the device callback, comes from Mikhail
// Kupchik's implementation in the audio work that has been pending as
// pull request #1478 since 2022.
...
AudioOutputWin32::AudioOutputWin32()
  : available(false), opened(false),
    sampleFormat(preferredSampleFormat), channels(preferredChannels),
    frequency(preferredFrequency), device(nullptr),
    ...
    doneBuffers(nullptr), buffersInFlight(0)
{
  ...
  fillWaveFormat(&wfx, sampleFormat, channels, frequency);
...
bool AudioOutputWin32::open()
{
  ...
  if (waveOutOpen(&device, WAVE_MAPPER, &wfx,
    ...
    vlog.error("Could not open audio playback device");
    available = false;
    return false;
  }
